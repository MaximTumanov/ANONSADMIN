...
class OurusersModelEvents extends JModel {
	...
	*/
	function getItems($lists, $limit, $limitstart){
		...
		if (isset($lists['filter_title']) && $lists['filter_title']) {
			$q .= " AND c.title LIKE '%{$lists['filter_title']}%' ";
		}
			
		if ($lists['date_from'] && $lists['date_to']) {
			$db->setQuery("SELECT GROUP_CONCAT(distinct id_event) FROM `{$this->eDate}` WHERE DATE(date) >= '{$lists['date_from']}' AND DATE(date) <= '{$lists['date_to']}'");
			$id_events = $db->loadResult();
			if (!$id_events) return null;
			$q .= " AND c.id_event IN({$id_events}) ";
		} else if($lists['date_from']) {
			$db->setQuery("SELECT GROUP_CONCAT(distinct id_event) FROM `{$this->eDate}` WHERE DATE(date) >= '{$lists['date_from']}'");
			$id_events = $db->loadResult();
			if (!$id_events) return null;
			$q .= " AND c.id_event IN({$id_events})";			
		}		
	...
	*/
	...
	function _save($title, $alias, $img, $s_desc, $desc, $address, $type, $vip, $wtf, $published, $category, $id_place, $dates, $k_title, $k_desc, $k_keyw, $price, $vk_video) {
		$db = &JFactory::getDBO();
		$q = "INSERT INTO `{$this->tEvents}` VALUES('', '{$title}', '{$alias}', '{$img}', '{$s_desc}', '{$desc}', '{$address}', '{$type}', '{$vip}', '{$wtf}', '{$published}', '{$k_title}', '{$k_desc}', '{$k_keyw}', '{$price}', '{$vk_video}')";
		$db->setQuery($q);
		$db->query();
	 ...
	 */
	function _update($id_event, $title, $alias, $img, $s_desc, $desc, $address, $type, $vip, $wtf, $published, $category, $id_place, $dates, $k_title, $k_desc, $k_keyw, $price, $vk_video){
		$db = &JFactory::getDBO();
	...
	function _check_in_base($alias){
		$db = &JFactory::getDBO();
		$q = "SELECT count(*) FROM `#__events` WHERE `alias` = '{$q}'";
		$db->setQuery($q);
		return $db->loadResult();
	}
