...
class JHTML
{
	 ...
	 * @since	1.5
	 */
	function image($url, $alt, $fixSize = null, $attribs = null)
	{
		if (is_array($attribs)) {
			$attribs = JArrayHelper::toString( $attribs );
		}
		...
		return "<img src='{$url}' alt='{$alt}' title='{$alt}' '{$attribs}' />";
	}
	...
	/**
	* отображает постраницную навигацию
	* @param int    $page   текущая страница, по умолчанию 1
	* @param int    $all    кол-во всех записей
	* @param int    $limit  кол-во материалов, отображающих на странице, по умолчанию 1
	* @param string $option 
	* @return pagination or empty
	*/
	function pagination($page=1, $all, $limit=1, $option)
	{
		$html = '';
		if( ceil($all/$limit) > 1 ):
			...
			$pervpage_link = JRoute::_('index.php?'.$option.'&page=1');
			$prevpage_link = JRoute::_('index.php?'.$option.'&page='.($page-1));
			$nextpage_link = JRoute::_('index.php?'.$option.'&page='.($page+1));
			$lastpage_link = JRoute::_('index.php?'.$option.'&page='.$total);
			
			$page2left_link = JRoute::_('index.php?'.$option.'&page='.($page-2));
			$page1left_link = JRoute::_('index.php?'.$option.'&page='.($page-1));
			$page2right_link = JRoute::_('index.php?'.$option.'&page='.($page+2));
			$page1right_link = JRoute::_('index.php?'.$option.'&page='.($page+1));	
		
		
		$html .= '<div class="pager"><a class="arrow_red_left" href="'.$page1left_link.'" title="На страницу '.($page-1).'"></a><form action="'.JRoute::_('index.php?'.$option).'" method="post">';
		
			if ($page != 1):
				$pervpage = '<a class="pager_left" href="'.$page1left_link.'" title="На страницу '.($page-1).'"></a>';
			endif;
			if ($page != $total):
				$nextpage = '<a class="pager_right" href="'.$page1right_link.'" title="На страницу '.($page+1).'"></a>';
			endif;
			$page2left = ( $page - 2 > 0 )
								? '<a href="'.$page2left_link.'" title="На страницу '.($page-2).'">'.($page-2).'</a>'
								: '<a href="">&nbsp;</a>'
								;
			$page1left = ( $page - 1 > 0 ) 
							 ? '<a href="'.$page1left_link.'" title="На страницу '.($page-1).'">'.($page-1).'</a>'
							 : '<a href="">&nbsp;</a>'
							 ;
			$page2right = ($page + 2 <= $total)
								? '<a href="'.$page2right_link.'" title="На страницу '.($page+2).'">'.($page+2).'</a>'
								: '<a href="">&nbsp;</a>'
								;
			$page1right = ( $page + 1 <= $total )
								? '<a href="'.$page1right_link.'" title="На страницу '.($page+1).'">'.($page+1).'</a>'
								: '<a href="">&nbsp;</a>'
								;		
			$html .=  $pervpage
					 . '<div class="pager_number_left">'
					 . $page2left
					 . $page1left
					 . '</div>'
					 . '<input id="paginationInput" class="pager_number_active" type="text" onkeyup="return getPage(\'paginationInput\', this.value, \''.$total.'\')" name="page" value="'.$page.'">'
					 . '<div class="pager_number_right">'
					 . $page1right
					 . $page2right
					 . '</div>'
					 . $nextpage;
		$html .= '</form><a class="arrow_red_right" href="'.$page1right_link.'" title="На страницу '.($page+1).'"></a></div>';
		endif;
		
		return $html;
	}

	function MegaImg ($folder, $image, $class = '', $style = '', $title = '', $attr = '') {
		$base = JURI::base();
		$img = "<img class='{$class}' style='{$style}' src='{$base}images/sunny/{$folder}/{$image}{$attr}' title='{$title}' />";
		return $img;
	}
}
?>