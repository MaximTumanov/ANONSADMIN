...
class JString
{
	...
	*/
	...
	/**
	* обрезает строку по кол-ву указанных слов
	* @param  string $text       текст, который необходимо обрезать
	* @param  int    $counttext  кол-во слов, по умолчанию 10
	* @param  string $sep        разделитель, по умолчанию пробел
	* @return sting              обрезанная строка + '...'
	*/
	function cropstr($text, $counttext = 10, $sep = ' ') {
	...
	/**
	* заменяет строку
	* @param array  $search array('что меняем' => 'на что меняем',....)
	* @param string $string исходная строка
	* @return string
	*/
	function replaceStrData($search, $string){
		if( is_array($search) ):
	...
	/**
	 * определяет расширение по MIME
	 * @param string $mime MIME-TYPE файла,
	 * для которого необходимо определить расширение
	 */
	function extSpot($mime){
		switch( $mime ):
			case'image/png':
				$ext = '.png';
				break;
	...
	function getWTF($i) {
		...
		return "<sup class='sup-{$i}'>{$array[$i]}</sup>";
	}
}
